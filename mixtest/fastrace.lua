...
local function hopping(dst_ip,ttl,try,ping_result)
	local ping_condvar = nmap.condvar(ping_result)
	...
	if prober_type =='ping' then
		...
		elseif ip_file then 		--从文件读入
			...
			for line in io.lines(ip_file) do
				...
				if #ip_list >= 15 then
					print('begin thread ping',ip_count)
					for i in pairs(ip_list) do
						local ping_co = stdnse.new_thread(hopping,ip_list[i],64,2,ping_result)
						ping_thread_handler[ping_co]=true
					end
					
				    repeat
				        for thread in pairs(ping_thread_handler) do
				            if coroutine.status(thread) == "dead" then
				                ping_thread_handler[thread] = nil
				            end
				        end
				        if (next(ping_thread_handler)) then
				            ping_condvar "wait"
				        end
				    until next(ping_thread_handler) == nil
				    ip_list={}
				end--end of if #ip_list>=15
			end--end of for
			--处理剩余不足15个ip
			print('begin thread ping',ip_count)
			for i in pairs(ip_list) do
				local last_hop_co = stdnse.new_thread(hopping,ip_list[i],iface,last_hop_result)
				last_hop_thread_handler[last_hop_co]=true
			end
