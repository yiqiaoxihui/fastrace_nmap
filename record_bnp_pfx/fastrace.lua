...
local function reverse_traceroute(trace,cmptrace)
	...
	while ttl ~=0 do
        ...
        if cmptrace ~= nil and cmptrace['start'] <= ttl and cmptrace['end'] >= ttl and cmptrace['hop'][ttl]~=0 and cmptrace['hop'][ttl] == trace['hop'][ttl] then
        	trace['BNP_LEN']=cmptrace['end']-ttl
        	trace['BNP']=ttl
        	trace['start'] = ttl
...
local function test_max_fpx(new_link,new_node,pfx,new_router,bnp_len)
	for i=1, 31 do
		if pfx <=i then
	...
	if BNP_PFX_LEN[fpx]~= nil then
		if bnp_len~=-1 then
			BNP_PFX_LEN[fpx]['sum']=BNP_PFX_LEN[fpx]['sum']+1
			BNP_PFX_LEN[fpx]['bnp_len']=BNP_PFX_LEN[fpx]['bnp_len']+bnp_len
		end
	else
		BNP_PFX_LEN[fpx]={}
		BNP_PFX_LEN[fpx]['sum']=0
		BNP_PFX_LEN[fpx]['bnp_len']=0
		if bnp_len~=-1 then
			BNP_PFX_LEN[fpx]['sum']=BNP_PFX_LEN[fpx]['sum']+1
			BNP_PFX_LEN[fpx]['bnp_len']=BNP_PFX_LEN[fpx]['bnp_len']+bnp_len
		end
...
local function get_new_link_node_number(trace,pfx)
	...
	--测试不同前缀下的边，节点获取情况
	test_max_fpx(new_node,new_link,pfx,new_router,trace['BNP_LEN'])
	if DEBUG == 1 or VERBOSE>=1 then
...
action=function(host)
    ...
    if OUTPUT_TYPE == "file" then
		...
		for k,v in pairs(BNP_PFX_LEN) do
			OUTPUT_FILE_HANDLER:write("BNP_PFX_LEN: ",k," ",v['bnp_len'],v['sum'],"\n")
		end
